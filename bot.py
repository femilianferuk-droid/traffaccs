...
EMOJI = {
    ...
    "stats_grow": "5870930636742595124",     # 📊 рост
...
class Database:
    ...
    async def mark_code_received(self, purchase_id: int) -> None:
        """Помечает, что код был получен"""
        async with self.pool.acquire() as conn:
            await conn.execute(
                "UPDATE purchases SET code_received = TRUE WHERE id = $1",
                purchase_id
            )
    ...
    async def set_price(
        ...
        """Устанавливает цену для типа и страны"""
        async with self.pool.acquire() as conn:
            await conn.execute(
    ...
    # ==================== YOOMONEY ====================
    async def add_yoomoney_payment(
        self,
        label: str,
        user_id: int,
        amount: Decimal,
        ...
        """Добавляет YooMoney платёж"""
        async with self.pool.acquire() as conn:
            await conn.execute(
                """INSERT INTO yoomoney_payments 
                   (label, user_id, amount, is_topup, account_type, country)
                   VALUES ($1, $2, $3, $4, $5, $6)""",
                label, user_id, amount, is_topup, account_type, country
            )

    async def update_yoomoney_payment(
        self, label: str, status: str
    ) -> Optional[Dict[str, Any]]:
        """Обновляет статус YooMoney платежа"""
        async with self.pool.acquire() as conn:
            return await conn.fetchrow(
                "UPDATE yoomoney_payments SET status = $1 WHERE label = $2 RETURNING *",
                status, label
            )

    async def get_pending_yoomoney_payments(self) -> list:
        """Получает все незавершённые YooMoney платежи"""
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM yoomoney_payments WHERE status = 'pending'"
            )
            return [dict(row) for row in rows]
...
async def payment_methods_keyboard(
    ...
    price = await db.get_price(account_type, country) or {
        "price_rub": 100,
        "price_usdt": 1.2,
        "price_ton": 1.2
    }
...
async def get_code_callback(callback: types.CallbackQuery):
    ...
    if session_string:
        ...
        if code:
            ...
            await db.mark_code_received(purchase_id)
...
async def topup_crypto_callback(callback: types.CallbackQuery):
    """Обработчик пополнения через Crypto Bot"""
    text = (
        f'<b><tg-emoji emoji-id="{EMOJI["cryptobot"]}">👾</tg-emoji> Пополнение через Crypto Bot</b>\n\n'
        f'Используйте команды:\n'
        f'<code>/topup_usdt сумма</code> - пополнить в USDT\n'
        f'<code>/topup_ton сумма</code> - пополнить в TON\n\n'
        f'<tg-emoji emoji-id="{EMOJI["info"]}">ℹ</tg-emoji> Будет создан счёт в Crypto Bot для оплаты'
    )
    await callback.message.edit_text(
        text, reply_markup=back_to_profile_keyboard()
    )
...
async def cmd_topup_usdt(message: types.Message):
    """Команда пополнения в USDT"""
    if not crypto_pay:
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Crypto Bot не настроен'
        )
        return
    ...
    try:
        ...
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["info"]}">ℹ</tg-emoji> Использование: <code>/topup_usdt сумма</code>'
        )
        return
    ...
    if invoice:
        ...
        text = (
            f'<b><tg-emoji emoji-id="{EMOJI["cryptobot"]}">👾</tg-emoji> Счёт создан!</b>\n\n'
            f'<a href="{pay_url}">Нажмите для оплаты</a>\n\n'
            f'Сумма: {amount} USDT\n'
            f'<tg-emoji emoji-id="{EMOJI["loading"]}">🔄</tg-emoji> Баланс пополнится автоматически после оплаты'
        )
        await message.answer(text)
    else:
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Ошибка создания счёта'
        )


@dp.message(Command("topup_ton"))
async def cmd_topup_ton(message: types.Message):
    """Команда пополнения в TON"""
    if not crypto_pay:
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["cross"]}">❌</tg-emoji> Crypto Bot не настроен'
        )
    ...
    try:
        ...
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["info"]}">ℹ</tg-emoji> Использование: <code>/topup_ton сумма</code>'
        )
        return
...
async def topup_yoomoney_callback(callback: types.CallbackQuery):
    """Обработчик пополнения через YooMoney"""
    text = (
        f'<b><tg-emoji emoji-id="{EMOJI["money"]}">🪙</tg-emoji> Пополнение через YooMoney</b>\n\n'
        f'Используйте команду:\n'
        f'<code>/topup_ym сумма</code>\n\n'
        f'Или переведите напрямую на кошелёк:\n'
        f'<code>{YOOMONEY_WALLET}</code>\n'
        f'и отправьте чек в @VestSupport'
    )
    await callback.message.edit_text(
        text, reply_markup=back_to_profile_keyboard()
    )
...
@dp.message(Command("topup_ym"))
async def cmd_topup_ym(message: types.Message):
    """Команда пополнения через YooMoney"""
    try:
        parts = message.text.split()
        if len(parts) < 2:
            raise ValueError
        ...
        await message.answer(
            f'<tg-emoji emoji-id="{EMOJI["info"]}">ℹ</tg-emoji> Использование: <code>/topup_ym сумма</code>'
        )
        return
    ...
    await db.add_yoomoney_payment(
        label, message.from_user.id, amount, is_topup=True
    )
...
async def pay_balance_callback(callback: types.CallbackQuery):
    ...
    price = await db.get_price(account_type, country) or {"price_rub": 100}
    ...
    text = (
        ...
        f'<tg-emoji emoji-id="{EMOJI["wallet"]}">👛</tg-emoji> <b>Остаток:</b> {user["balance"] - Decimal(str(price["price_rub"]))} ₽\n'
...
async def pay_usdt_callback(callback: types.CallbackQuery):
    ...
    price = await db.get_price(account_type, country) or {"price_usdt": 1.2}
    ...
    if invoice:
        await db.add_crypto_payment(
            ...
            Decimal(str(price["price_usdt"])), "USDT", account_type, country
        )
        ...
        text = (
            ...
            f'<a href="{pay_url}">Нажмите для оплаты</a>\n\n'
            f'<tg-emoji emoji-id="{EMOJI["loading"]}">🔄</tg-emoji> Ожидание оплаты...\n'
...
async def pay_ton_callback(callback: types.CallbackQuery):
    ...
    price = await db.get_price(account_type, country) or {"price_ton": 1.2}
    ...
    if invoice:
        await db.add_crypto_payment(
            str(invoice["invoice_id"]), callback.from_user.id,
            Decimal(str(price["price_ton"])), "TON", account_type, country
        )
...
async def pay_yoomoney_callback(callback: types.CallbackQuery):
    ...
    payment_link = create_yoomoney_link(
        Decimal(str(price["price_rub"])), label,
        f"Покупка {ACCOUNT_TYPES[account_type]} {COUNTRY_NAMES[country]}"
    )

    await db.add_yoomoney_payment(
        label, callback.from_user.id, Decimal(str(price["price_rub"])),
        account_type=account_type, country=country
    )
...
async def check_crypto_payments_loop():
    """Фоновый цикл проверки крипто-платежей"""
    while True:
...
# ==================== ПРОВЕРКА YOOMONEY ПЛАТЕЖЕЙ ====================
async def check_yoomoney_payments_loop():
    """Фоновый цикл проверки YooMoney платежей (пока ручная)"""
    while True:
        await asyncio.sleep(60)
        # YooMoney требует ручного подтверждения через поддержку
        # или вебхук, который сложно реализовать без сервера
        pass
...
async def chprice_country_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    data = await state.get_data()
    current_price = await db.get_price(data["change_price_type"], country) or {
        "price_rub": 0,
        "price_usdt": 0,
        "price_ton": 0
    }
    ...
    text = (
        ...
        f'Текущие цены:\n'
        f'• Рубли: {current_price["price_rub"]} ₽\n'
        f'• USDT: {current_price["price_usdt"]}\n'
        f'• TON: {current_price["price_ton"]}\n\n'
...
async def chprice_rub_handler(message: types.Message, state: FSMContext):
    """Обработчик получения цены в рублях"""
    try:
        price_rub = Decimal(message.text.strip())
        await state.update_data(change_price_rub=price_rub)
...
async def chprice_usdt_handler(message: types.Message, state: FSMContext):
    """Обработчик получения цены в USDT"""
    try:
        price_usdt = Decimal(message.text.strip())
        await state.update_data(change_price_usdt=price_usdt)
...
async def chprice_ton_handler(message: types.Message, state: FSMContext):
    """Обработчик получения цены в TON и сохранения"""
    try:
        price_ton = Decimal(message.text.strip())
        data = await state.get_data()

        # Сохраняем цену для конкретной страны и типа
        await db.set_price(
            data["change_price_type"],
            data["change_price_country"],
            data["change_price_rub"],
            data["change_price_usdt"],
            price_ton
        )
        ...
        await message.answer(
            ...
            f'Новые цены:\n'
            f'• Рубли: {data["change_price_rub"]} ₽\n'
            f'• USDT: {data["change_price_usdt"]}\n'
            f'• TON: {price_ton}',
            reply_markup=admin_panel_keyboard()
        )
        await state.clear()
...
async def main():
    ...
    asyncio.create_task(check_yoomoney_payments_loop())
