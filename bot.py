...
class Database:
    ...
    async def init_tables(self):
        async with self.pool.acquire() as conn:
            ...
            try:
                await conn.execute("ALTER TABLE users ADD COLUMN IF NOT EXISTS sales_count INT DEFAULT 0")
            except Exception:
                pass
            try:
                await conn.execute("ALTER TABLE purchases ADD COLUMN IF NOT EXISTS seller_id BIGINT")
            except Exception:
                pass
            try:
                await conn.execute("ALTER TABLE purchases ADD COLUMN IF NOT EXISTS commission DECIMAL DEFAULT 0")
            except Exception:
                pass

            # Индексы
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_accounts_available ON accounts(is_sold, account_type, country)")
            await conn.execute("CREATE INDEX IF NOT EXISTS idx_crypto_pending ON crypto_payments(status)")
    ...
    async def mark_sold(self, aid: int, uid: int) -> None:
        async with self.pool.acquire() as conn:
            await conn.execute(
    ...
    async def update_code(self, aid: int, code: str) -> None:
        async with self.pool.acquire() as conn:
    ...
    async def get_purchases(self, uid: int) -> List[Dict[str, Any]]:
        async with self.pool.acquire() as conn:
    ...
    async def get_purchase(self, pid: int, uid: int) -> Optional[Dict[str, Any]]:
        async with self.pool.acquire() as conn:
    ...
    async def add_withdraw(self, uid: int, amount: Decimal, sbp: str) -> None:
        async with self.pool.acquire() as conn:
            await conn.execute(
    ...
    async def get_mirrors(self, uid: int) -> List[Dict[str, Any]]:
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT * FROM mirror_bots WHERE owner_id = $1 AND is_active = TRUE", uid
            )
            return [dict(row) for row in rows]
...
# =====================================================================
# КЛАВИАТУРЫ (все кнопки со стилями: primary/success/danger)
# =====================================================================
...
def account_type_keyboard(prefix: str) -> InlineKeyboardMarkup:
    keyboard = []
...
def countries_keyboard(prefix: str, account_type: str, page: int = 0, per_page: int = 8) -> InlineKeyboardMarkup:
...
async def profile_callback(callback: CallbackQuery):
    ...
    text = (
        ...
        f'<tg-emoji emoji-id="{EMOJI["box"]}">📦</tg-emoji> '
        f'<b>Покупок:</b> {user.get("purchases_count", 0)}\n'
        f'<tg-emoji emoji-id="{EMOJI["money_send"]}">🪙</tg-emoji> '
        f'<b>Продаж:</b> {user.get("sales_count", 0)}'
    )
...
async def get_code_callback(callback: CallbackQuery):
    ...
    purchase = await db.get_purchase(purchase_id, callback.from_user.id)
    ...
    if session_string:
        ...
        if code:
            await db.update_code(purchase["account_id"], code)
    ...
    if invoice and invoice.get("status") == "paid":
        ...
        if payment["is_topup"]:
            ...
            if account and not account["is_sold"]:
                await db.mark_sold(account["id"], payment["user_id"])
...
async def process_withdraw_amount(message: Message, state: FSMContext):
    ...
    if amount > user.get("balance", 0):
        await message.answer(
            f"Недостаточно средств. Баланс: {user.get('balance', 0)} ₽"
        )
        return
...
async def process_withdraw_sbp(message: Message, state: FSMContext):
    ...
    await db.add_withdraw(message.from_user.id, amount, sbp_details)
...
async def mirror_list_callback(callback: CallbackQuery):
    mirrors = await db.get_mirrors(callback.from_user.id)
...
async def sell_account_callback(callback: CallbackQuery):
    await callback.message.edit_text(
        ...
        reply_markup=account_type_keyboard("sell_type")
    )
    await callback.answer()
...
async def buy_account_callback(callback: CallbackQuery):
    await callback.message.edit_text(
        ...
        f'<tg-emoji emoji-id="{EMOJI["tag"]}">🏷</tg-emoji> '
        f'Выберите тип аккаунта:',
        reply_markup=account_type_keyboard("buy_type")
    )
    await callback.answer()
...
async def pay_balance_callback(callback: CallbackQuery):
    ...
    if user.get("balance", 0) < account["price_rub"]:
        await callback.answer(
            f"Недостаточно средств. Баланс: {user.get('balance', 0)} ₽",
            show_alert=True
        )
    ...
    await callback.message.edit_text(
        ...
        f'<tg-emoji emoji-id="{EMOJI["wallet"]}">👛</tg-emoji> '
        f'<b>Остаток:</b> {user.get("balance", 0) - account["price_rub"]} ₽\n'
